                            ...
                            <div x-data="{ copied: false }" class="rounded-2xl border border-slate-800 bg-slate-900 px-5 py-4">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    @if ($room->room_code)
                                        <button
                                            type="button"
                                            @click="navigator.clipboard.writeText('{{ $room->room_code }}'); copied = true; setTimeout(() => copied = false, 1500)"
                                            class="rounded-xl border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-300 transition hover:bg-slate-800 hover:text-white"
                                        >
                            ...
                            <div x-data="{ copied: false }" class="rounded-2xl border border-slate-800 bg-slate-900 px-5 py-4">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    @if ($room->room_password)
                                        <button
                                            type="button"
                                            @click="navigator.clipboard.writeText('{{ $room->room_password }}'); copied = true; setTimeout(() => copied = false, 1500)"
                                            class="rounded-xl border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-300 transition hover:bg-slate-800 hover:text-white"
                                        >
                                            <span x-show="! copied">Copy</span>
                                            <span x-show="copied">Copied</span>
